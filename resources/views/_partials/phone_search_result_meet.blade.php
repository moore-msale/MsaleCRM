            ...
            <div class="collapse collapse-multi show" id="collapseAjax{{ $loop->index }}">
                @foreach($items as $value)
                    @if(auth()->user()->role=='admin')
                        <div class="nav-link products px-2 close-search" data-toggle="modal" data-target="#EditMeetAdmin-{{$value->id}}">
                            @else
                                <div class="nav-link products px-2 close-search" data-toggle="modal" data-target="#EditMeet-{{$value->id}}">
                                    @endif
