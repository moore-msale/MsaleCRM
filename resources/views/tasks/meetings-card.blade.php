<div class="mt-3 mx-lg-3 mx-0 px-3 py-3 work-desk position-relative potencial-show" id="meet-{{$meeting->id}}" >
 @dd($meeting)
    ...
    <div class="meet-company pb-1 position-relative" style="border-bottom:1px solid rgba(0,0,0,0.2);">
        <div class="font-weight-bold meet-company sf-medium"> {{ $meeting->taskable->customer->company }} </div>
        ...
        <div class="btn-group dropleft col-1 position-absolute" style="top:0%; right:-4%;">
            ...
            <div class="dropdown-menu pl-2 shadow" style="border-radius: 0px; border:none;">
                <p class="mb-0 drop-point sf-medium pl-2" data-toggle="modal" data-target="#EditMeetAdmin-{{$meeting->id}}" style="cursor:pointer;">изменить</p>
                <p class="mb-0 drop-point sf-medium pl-2" data-toggle="modal" data-target="#DeleteMeetAdmin-{{$meeting->id}}" style="cursor:pointer;">удалить</p>
            </div>
        </div>
    </div>
    <div class="d-flex mt-2">
        ...
        <div class="deal-text sf-bold mb-0 d-flex align-items-start ml-2">
            <svg class="svg" width="13" height="13" viewBox="0 0 11 11" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path class="change-color" d="M5.49771 0.916664C2.96542 0.916664 0.916672 2.96771 0.916672 5.5C0.916672 8.03229 2.96542 10.0833 5.49771 10.0833C8.03001 10.0833 10.0833 8.03229 10.0833 5.5C10.0833 2.96771 8.03001 0.916664 5.49771 0.916664ZM5.50001 9.16667C3.47417 9.16667 1.83334 7.52583 1.83334 5.5C1.83334 3.47416 3.47417 1.83333 5.50001 1.83333C7.52584 1.83333 9.16667 3.47416 9.16667 5.5C9.16667 7.52583 7.52584 9.16667 5.50001 9.16667ZM5.72917 3.20833H5.04167V5.95833L7.44563 7.40208L7.79167 6.83833L5.72917 5.61458V3.20833Z" fill="{{ isset($meeting->status) != null ? $meeting->status->color : '#C4C4C4'}}"/>
            </svg><span class="pl-1 meet-date2 change-color" style="{{ isset($meeting->status) != null ? 'color:'.$meeting->status->color.';' : 'color:#C4C4C4;'}}">
                {{ \Carbon\Carbon::parse($meeting->deadline_date)->format('H:i') }}
            </span>
        </div>
        <div class="deal-text sf-bold mb-0 d-flex align-items-start ml-2">
            ...
            <span class="pl-1 meet-name">
                {{ $meeting->taskable->customer->name }}
            </span>
        </div>
    </div>
    {{--<div class="ml-auto">--}}
    ...
    {{--</div>--}}
    <div class="status-meet position-absolute h-100" style="width:3px; background-color: {{ isset($meeting->status) != null ? $meeting->status->color : '#C4C4C4'}}; top:0%; left:0%; "></div>
</div>
@foreach($meeting as $task)
